...
void adamw_step_cpu(float* param, float* grad, float* m, float* v, float* scratch, std::size_t n,
                    float lr, float beta1, float beta2, float bias1, float bias2, float eps,
                    float weight_decay) {
...
float clip_grad_norm_cpu(float** grads, const std::size_t* sizes, std::size_t n_tensors,
                         float max_norm) {
  double total = 0.0;
