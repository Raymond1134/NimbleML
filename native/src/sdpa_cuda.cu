...
namespace nimbleml {
  ...
  namespace {
    ...
    __global__ void gelu_backward_kernel(const float* grad, const float* x, const float* tanh_u, float* grad_x, std::size_t n) {
      const std::size_t i = blockIdx.x * blockDim.x + threadIdx.x;
      if (i >= n) return;
      const float k = 0.7978845608028654f;
    ...
    __global__ void flash_sdpa_bwd_kernel(const float* grad_out, const float* q, const float* k, const float* v, const float* m_in,
                                          const float* l_in, float* grad_q, float* grad_k, float* grad_v, int seq, int dk, float scale) {
      const int bh = blockIdx.y;
      const int i = blockIdx.x * blockDim.x + threadIdx.x;
      if (i >= seq) return;
    ...
    __global__ void rmsnorm_fwd_kernel(const float* x, const float* gamma, float* out, float* ms, float* rms, int rows, int dim, float eps) {
    ...
    __global__ void rmsnorm_bwd_kernel(const float* grad, const float* x, const float* gamma, const float* ms, const float* rms,
                                      float* grad_x, float* grad_gamma, int rows, int dim, float eps) {
      const int row = blockIdx.x;
      if (row >= rows) return;
      const float* xr = x + static_cast<std::size_t>(row) * dim;
  ...
  void rmsnorm_backward_cuda(const float* grad, const float* x, const float* gamma, const float* ms, const float* rms,
    ...
    rmsnorm_bwd_kernel<<<static_cast<int>(rows), threads>>>(
        grad, x, gamma, ms, rms, grad_x, grad_gamma, static_cast<int>(rows), static_cast<int>(dim), eps
      );
  ...
  void flash_sdpa_forward_cuda(const float* q, const float* k, const float* v, float* out, float* m,
    ...
    if (dk > 256) throw std::runtime_error("flash_sdpa_forward_cuda: dk > 256 not supported");
    dim3 block(32);
    dim3 grid(static_cast<unsigned>((seq + 31) / 32), static_cast<unsigned>(bh));
    const std::size_t shmem = 32 * dk * sizeof(float);
    flash_sdpa_fwd_kernel<<<grid, block, shmem>>>(q, k, v, out, m, l, static_cast<int>(seq), static_cast<int>(dk), scale);
    check_cuda(cudaGetLastError(), "flash_sdpa_forward_cuda");
  }

  void flash_sdpa_backward_cuda(const float* grad_out, const float* q, const float* k, const float* v, const float* m, 
                                const float* l, float* grad_q, float* grad_k, float* grad_v, std::size_t bh,
                                std::size_t seq, std::size_t dk, float scale) {
    ...
    dim3 block(32);
    dim3 grid(static_cast<unsigned>((seq + 31) / 32), static_cast<unsigned>(bh));
    flash_sdpa_bwd_kernel<<<grid, block>>>(grad_out, q, k, v, m, l, grad_q, grad_k, grad_v, static_cast<int>(seq), static_cast<int>(dk), scale);
