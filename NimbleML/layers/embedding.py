...
class Embedding(Module):
    ...
    def forward(self, inputs):
        ...
        W = self.weights._view((self.vocab_size, self.embed_dim))
        ...
        def _backward():
            if output.grad is None or not self.weights.requires_grad:
                return
            grad_flat = _save_for_backward(output.grad.reshape(-1, self.embed_dim))
    ...
    def forward_prefix(self, seq_len: int):
        ...
        W = self.weights._view((self.vocab_size, self.embed_dim))
        # Prefix slice is a view into the embedding table (no copy). Safe until
        # optimizer.step(); GPT only reads it when building token+pos residuals.
        out = W[:seq_len]
        ...
        def _backward():
            if output.grad is None or not self.weights.requires_grad:
                return
            grad_flat = _grad_out(output, (seq_len, self.embed_dim))
